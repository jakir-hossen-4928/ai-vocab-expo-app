...
export default function ChatScreen() {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (!apiKey) {
                ...
                setMessages((prev) => [
                    ...prev,
                    {
                        id: Date.now().toString(),
                        role: 'assistant',
                        content: 'Please set your OpenRouter API Key in Settings to chat with AI.',
                    },
                ]);
                return;
            }
            ...
            const response = await chatWithVocabulary(vocabulary, history, apiKey, model);
    ...
    const renderMessage = ({ item }: { item: Message }) => {
        ...
        return (
            <View
                accessible={true}
                accessibilityLabel={`${isUser ? 'You said' : 'AI said'}: ${item.content}`}
                accessibilityRole="text"
                style={[
                    styles.messageBubble,
                    isUser ? styles.userBubble : styles.botBubble,
                    { backgroundColor: isUser ? colors.primary : colors.card, borderColor: colors.border }
                ]}
            >
                {!isUser && (
                ...
                <Text selectable style={[styles.messageText, { color: isUser ? '#fff' : colors.text }]}>
                    {item.content}
                </Text>
            </View>
        );
