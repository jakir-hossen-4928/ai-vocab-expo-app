 ...
 */
export const LexicalMarkdownRenderer = ({ markdown }: LexicalMarkdownRendererProps) => {
    ...
    const processedMarkdown = useMemo(() => {
        ...
        const bareImageUrlRegex = /(^|\s)(https?:\/\/[^\s$.?#].[^\s]*\.(?:jpg|jpeg|gif|png|webp|bmp))(\s|$)/gi;

        return markdown.replace(bareImageUrlRegex, (match, p1, p2, p3) => {
    ...
    const customRenderers = {
        ...
        paragraph: ({ children }: any) => {
            const RNText = require('react-native').Text;
            return <RNText style={(customStyles as any).paragraph}>{children}</RNText>;
        },
    };
